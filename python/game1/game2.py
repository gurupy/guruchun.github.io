...
def run_game():
    ...
    while not quit:
        ...
        for i, item in enumerate(answerList):
            if item < 14:
                drawObject(answers[item], (item*enemy_size, 700))
            else:
                drawObject(answers[item], ((item-14)*enemy_size, 730))
            i+=1
